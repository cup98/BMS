...
void CAN_Get_PreCfg(void)                                           //读取CAN报文内数据字段的数据给Pre配置
{
    if (CAN1_GetMsg(&CAN_PrechargeCfg) == 1)
    {
        ...
        Pre_Cfg_WriteCfg(PRE_CFG_PERCENT ,CAN_PrechargeCfg.data[3]);//data[3]字段赋值给percent
...
{
    CAN_NodeState.data[0] = (unsigned int)Node_StateBack(NODE_Element_CURRENT_STATE);//读取当前节点状态赋值到data[0]字段
    CAN_NodeState.data[1] = (unsigned int)Node_StateBack(NODE_Element_NEXT_STATE);   //读取下一节点状态赋值到data[1]字段
    CAN_NodeState.data[2] = (unsigned int)RelayM_CtrlData[0].ctrl_status;            //读取当前节点状态赋值到data[0]字段
    CAN_NodeState.data[3] = (unsigned int)RelayM_CtrlData[1].ctrl_status;            //读取下一节点状态赋值到data[1]字段
    CAN_NodeState.data[7] = (unsigned int)Node_StateBack(NODE_Element_CONDITION_NUM);//读取当前节点分支数赋值到data[2]字段
