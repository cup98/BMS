...
void CAN_Get_PreCfg(void)									//璇诲嚭鎺ュ彈鍒扮殑鏁版嵁鍐嶅彂閫佸嚭鏉
{
  	...
  	{
      	Pre_Cfg_WriteCfg(FAULT ,CAN_PrechargeCfg.data[0]);
      	Pre_Cfg_WriteCfg(CLOCK ,CAN_PrechargeCfg.data[1]);
      	Pre_Cfg_WriteCfg(PRE_VOLTAGE ,CAN_PrechargeCfg.data[2]);
...
{
	CAN_NodeData.data[0] = (unsigned char)Node_StateBack(CURRENT_NODE);
	CAN_NodeData.data[1] = (unsigned char)Node_StateBack(NEXT_NODE);
	CAN_NodeData.data[7] = (unsigned char)Node_StateBack(BRANCH_NUM);
